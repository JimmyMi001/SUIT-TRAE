/* =========================================================
   ...
   · 简体中文 README.md → 英文 README.en.md / 繁體 README.zh-TW.md
   ...
   · 用法：
       ...
       node scripts/translate-readme.js            # 执行翻译（en + zh-TW）
   ========================================================= */
...
const OUTS = {
  ...
  'zh-TW': path.join(ROOT, 'README.zh-TW.md')
};
const API = 'https://api.deepseek.com/v1/chat/completions';
const MODEL = 'deepseek-v4-flash';
const KEY = process.env.DEEPSEEK_KEY;
const TO_NAME = { en: 'English (US)', 'zh-TW': '繁體中文（中國香港）' };
...
/* ---------- 目标语言质量校验 ----------
   ...
   zh-TW: 检测简体特征双字词，出现 ≥2 个视为残留简体 → 需重翻 */
...
function systemPrompt(to) {
  ...
  const properNoun = isEn
    ? '- 品牌/平台名尽量给出英文常用名（高德→Amap、美团→Meituan、飞猪→Fliggy、途牛→Tuniu、携程→Ctrip、12306 保留数字、DeepSeek 保留）；城市名用拼音（北京→Beijing、广州→Guangzhou、杭州→Hangzhou）；学校名 Shenzhen Information Technology University。其余文件名/命令/API 保留原样'
...
**🌐 ${label}：<a href="./README.md">简体中文</a> · <a href="./README.zh-TW.md">繁體中文</a> · <a href="./README.en.md">English</a>**
...
async function main() {
  ...
  const langs = process.argv.includes('--en') ? ['en'] : process.argv.includes('--tw') ? ['zh-TW'] : ['en', 'zh-TW'];
